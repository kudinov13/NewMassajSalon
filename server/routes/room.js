...
roomRouter.get('/:roomId', requireAuth, async (req, res) => {
    const db = getDb();
    const a = await db.get('SELECT * FROM appointments WHERE roomId = ?', req.params.roomId);
...
roomRouter.post('/:roomId/signal', requireAuth, async (req, res) => {
    ...
    await db.run(
        ...
        req.params.roomId, req.user.id, type, JSON.stringify(data)
    );
    res.json({ ok: true });
});
...
roomRouter.get('/:roomId/signals', requireAuth, async (req, res) => {
    ...
    const signals = await db.all(
        ...
        req.params.roomId, req.user.id, afterId
    );
