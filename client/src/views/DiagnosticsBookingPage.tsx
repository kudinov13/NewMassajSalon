...
const DiagnosticsBookingPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    API.diagnostics.getAvailableSlots()
      .then(setSlots)
      .catch(() => {})
  ...
  const handleBook = async () => {
    ...
    try {
      setError("");
      await API.diagnostics.bookSlot({ slotId: selectedSlot.id, fullName: fullName.trim(), phone: phone.trim() });
      setSuccess(true);
    } catch (e: any) {
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="bg-white/70 rounded-[25px] p-10 text-center max-w-[500px]">
            ...
            <p className="[font-family:'Vela_Sans',sans-serif] font-light text-[#6B5744]/80 text-base mb-6">
              Диагностика: {formatDate(selectedSlot!.date)} в {selectedSlot!.time}
            </p>
    ...
    <div className="bg-[#efdec5] min-h-screen w-full">
      ...
      <div className="max-w-[800px] mx-auto px-4 sm:px-6 md:px-10 pb-16">
        ...
        <p className="[font-family:'Vela_Sans',sans-serif] font-light text-[#6B5744]/70 text-base mb-8">
          Выберите удобную дату и время для онлайн-консультации со специалистом по диагностике
        </p>
